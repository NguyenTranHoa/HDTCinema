...
class formBuilder
{
    ...
    function validate($name,$rules) // function for validation name: name of the field, rules: validation rules
    {
        ...
        else if(array_key_exists("max",$rules)) // if rules array have value max
        {
            ...
            $this->validators=$this->validators."stringLength: {
                    max: $max,
                    message: 'Trường $label phải nhỏ hơn $max kí tự'
                }," ;
        }
        else if(array_key_exists("min",$rules)) // if rules array have value min
        {
            ...
            $this->validators=$this->validators."stringLength: {
                    min: $min,
                    message: 'Trường $label phải lớn hơn $min kí tự'
                }," ;
        }
        if (array_key_exists("regexp",$rules)) // if rules array have value regular expression validation
        {
            ...
            switch($exp) // selecting regular expression types
            {
                case "name": // if validation for name
                    $expression='/^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂ ưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ ]+$/';
                    $err_msg="Trường $label chỉ là các chữ cái";
                    break;
                case "age":
                    $expression='/^[1-9][0-9]?$|^100$/';
                    $err_msg="Trường $label là các chữ số từ 1 đến 100";
                    break;
                case "address":
                ...
                case "mobile":
                    $expression='/^[0][0-9]{9}$/';
                    $err_msg="Trường $label bao gồm 10 con số bắt đầu từ số 0";
                    break;
                case "phone":
